...
logreg_mtl.py
──────────────────
Multi-Task Learning with Logistic Regression for COLX 581 Sprint 3.
Implements MTL with cascaded prediction using POS tags.

Concepts were and code templates were used from past lecture notes (DSCI 572 lecture 2; DSCI 571 lecture 6, 8; COLX 521 lecture 4, 7).. 
...
MTL — cascaded prediction:
    The secondary task (POS tagging) is performed using spaCy silver
    annotation. Its output is then used as input features for the primary task
    (opinion/misinformation classification). 
...
def _build_features(
    ...
    Add FastText embeddings and cascaded POS features too.
    Returns (X_train, X_eval) as sparse matrices.

    Features:
        1. TF-IDF      
        2. TF-IDF weighted FastText     
    ...
    # 4. POS distribution features
    pos_scaler = StandardScaler()
    train_pos = pos_scaler.fit_transform(_extract_pos_features(_extract_texts(train_rows)))
    eval_pos  = pos_scaler.transform(_extract_pos_features(_extract_texts(eval_rows)))

    # Combines sparse TF-IDF and dense linguistic features
    # Used Claude to ensure properly combine sparse and dense features
    # add csr_matrix for embeddings and POS features
    X_train = hstack([train_tfidf, csr_matrix(train_ling), csr_matrix(train_embeddings), csr_matrix(train_pos)])
    X_eval  = hstack([eval_tfidf,  csr_matrix(eval_ling),  csr_matrix(eval_embeddings),  csr_matrix(eval_pos)])
...
def run(
    ...
    # 2. Features
    print("  Building features (TF-IDF, FastText, linguistic, cascaded POS)")
