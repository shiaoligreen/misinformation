...
Usage:
    python cnn_mtl_no_ling.py

Usage (from notebook):
    import cnn_mtl_no_ling as cnn_mtl_pos
    vocab        = cnn_mtl_pos.build_vocab(train_rows, preprocess_fn)
    embed_matrix = cnn_mtl_pos.load_fasttext_vectors(path, vocab)
    loader       = cnn_mtl_pos.make_loader(rows, vocab, shuffle=True)
    model        = cnn_mtl_pos.TextCNN(len(vocab), embed_matrix).to(device)
    model        = cnn_mtl_pos.train_model(model, train_loader, dev_loader, train_rows, device)
    preds, labels, probs = cnn_mtl_pos.predict(model, loader, device)
    metrics = cnn_mtl_pos.evaluate_model(model, test_loader, test_rows, device, name="TextCNN — Test Set")
"""
...
class OpinionDataset(Dataset):
    ...
    def __init__(self, rows: list[dict], vocab: Vocabulary, tokenize_fn=preprocess):
        ...
        for row in rows:
            ...
            self.samples.append((ids, labels, pos))
...
def _collate(batch):
    ...
    Parameter: 
        batch = list of (ids, label, pos) tuples
    ...
    seqs, label_dicts, pos_list = zip(*batch)
...
class TextCNN(nn.Module):
    ...
    def __init__(self, vocab_size: int, embed_matrix: torch.Tensor):
        ...
        # pos concatenated after dropout, before linear classifier
        self.classifiers = nn.ModuleDict({
            t: nn.Linear(num_filters * len(CONFIG["filter_sizes"]) + len(POS_TAGS), 1)
            for t in TARGETS
        })
    ...
    def forward(self, x: torch.Tensor, pos: torch.Tensor):
        ...
        combined = torch.cat([shared, pos], dim=1)
        #Help from Claude on list comprehension 
...
def evaluate_model(model: nn.Module, loader: DataLoader, rows: list[dict], device: torch.device,
                   name: str = "TextCNN MTL (POS)", target: str = TARGETS[0]):
    """
...
def main():
    ...
    for target in TARGETS:
        evaluate_model(model, dev_loader, dev_rows, device,
                       name=f"TextCNN MTL (POS) — Dev Set ({target})", 
                       target=target)
