...
Annotated Linguistic Features:
    Added to TextCNN model for COLX 581 Sprint 3.

Multi-task Learning:
    Multi-Task Learning with TextCNN Classifier for COLX 581 Sprint 3.
    Implements MTL with prediction using POS tags.
...
Usage:
    python cnn_mtl_ling.py

Usage (from notebook):
    import cnn_mtl_ling as cnn_mtl_ling
    vocab        = cnn_mtl_ling.build_vocab(train_rows, preprocess_fn)
    embed_matrix = cnn_mtl_ling.load_fasttext_vectors(path, vocab)
    loader       = cnn_mtl_ling.make_loader(rows, vocab, shuffle=True)
    model        = cnn_mtl_ling.TextCNN(len(vocab), embed_matrix).to(device)
    model        = cnn_mtl_ling.train_model(model, train_loader, dev_loader, train_rows, device)
    preds, labels, probs = cnn_mtl_ling.predict(model, loader, device)
    metrics = cnn_mtl_ling.evaluate_model(model, test_loader, test_rows, device, name="TextCNN — Test Set")
"""
...
# Claude used for inclusive list
POS_TAGS = [
    "ADJ",   # adjectives  
    "ADP",   # adpositions
    "ADV",   # adverbs  
...
# Secondary task: POS distribution 
# Claude used to correct errors
def get_pos_distribution(text: str) -> np.ndarray:
    """
    Secondary task output: A vector with normalized POS tag distribution.

    Steps:
        1. Tokenize with the base regex 
        2. SpaCy for POS tags
        3. Count frequency of each tag in POS_TAGS
        4. Normalize by total token count 
    ...
    Returns
    -------
    np.ndarray  shape (len(POS_TAGS),)  normalized POS proportions
    """
    tokens = TOKENIZE_FN(text)
    if not tokens:
        return np.zeros(len(POS_TAGS), dtype=np.float32)
    ...
    # Normalize by number of spaCy tokens
    total = len(doc)
    vector = np.array(
        [counts[tag] / total for tag in POS_TAGS],
        dtype=np.float32
    )
    return vector

def _extract_pos_features(texts: list[str]) -> np.ndarray:
    """
    Secondary task (POS distribution).

    Returns
    -------
    np.ndarray  shape (n_docs, len(POS_TAGS))
        Each row is the normalized POS distributions.
    """
    print(f"  Running secondary task: POS tagging {len(texts)} documents …")
    matrix = np.array(
        [get_pos_distribution(text) for text in texts],
        dtype=np.float32
    )
    print(f"  POS distribution feature dim: {matrix.shape[1]} tags")
    return matrix
...
class OpinionDataset(Dataset):
    ...
    def __init__(self, rows: list[dict], vocab: Vocabulary, tokenize_fn=preprocess):
        ...
        for row in rows:
            ...
            # add pos tagging, running once at dataset construction
            pos = torch.tensor(get_pos_distribution(row["text"]), dtype=torch.float)
            # add annotated linguistic features
            ling = torch.tensor([float(row.get(f, 0) or 0) for f in LINGUISTIC_FEATURES],
                                dtype=torch.float)
            self.samples.append((ids, labels, pos, ling))
...
def _collate(batch):
    ...
    Parameter: 
        batch = list of (ids, labels, pos, ling) tuples
    
    Returns tuple of: 
        padded: token id sequences padded, 
        labels: dict of {target: (B, )} tensors, one per classification target
        pos:    tensor of POS distribution features, shape (B, 15)
        ling:   tensor of linguistic features, shape (B, 6)
                     
    """
    # unpack list of (ids, labels_dict) tuples
    seqs, label_dicts, pos_list, ling_list = zip(*batch)
    ...
    # stack each target's pos tags
    pos = torch.stack(pos_list)
    # stack each target's linguistic features
    ling = torch.stack(ling_list)

    return padded, labels, pos, ling
...
class TextCNN(nn.Module):
    ...
    def __init__(self, vocab_size: int, embed_matrix: torch.Tensor):
        ...
        # pos and ling concatenated after dropout, before linear classifier
        self.classifiers = nn.ModuleDict({
            t: nn.Linear(
                num_filters * len(CONFIG["filter_sizes"]) + len(POS_TAGS) + len(LINGUISTIC_FEATURES), 1
                )
            for t in TARGETS
        })

    def forward(self, x: torch.Tensor, pos: torch.Tensor, ling: torch.Tensor):
        """
        Parameters: 
            x:  is the padded batch of token IDs after _collate. Size is batch x sequence length
            pos: normalized POS tag distribution per document. Size is batch x 15
            ling: linguistic feature values per document. Size is batch x 6 
        Returns: 
            dict of {target: (B, )} containing one raw logit per example in the batch. 
        ...
        combined = torch.cat([shared, pos, ling], dim=1)
        #Help from Claude on list comprehension 
        # each head produces one logit per example — returns {target: (B,)}
        return {t: self.classifiers[t](combined).squeeze(1) for t in TARGETS}
...
def train_model( model: nn.Module, train_loader: DataLoader, dev_loader: DataLoader, train_rows: list[dict], device: torch.device, train_targets: list[str] = None):
    ...
    for epoch in range(1, CONFIG["max_epochs"] + 1):
        ...
        #inner training loop
        for x, y, pos, ling in train_loader:
            # x: (B, T) — y: {target: (B,)}
            x = x.to(device)
            ...
            logits = model(x, pos, ling)  # {target: (B,)}
...
def evaluate_model(model: nn.Module, loader: DataLoader, rows: list[dict], device: torch.device,
                   name: str = "TextCNN MTL (POS + linguistic)", target: str = TARGETS[0]):
    """
...
def main():
    ...
    for target in TARGETS:
        evaluate_model(model, dev_loader, dev_rows, device,
                       name=f"TextCNN MTL (POS + linguistic) — Dev Set ({target})", 
                       target=target)
