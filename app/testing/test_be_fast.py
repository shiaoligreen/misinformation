...
# shared state between tests from causing the 500-exception test to fail
# when be_search.ix is already set (or None) from a previous test.
@pytest.fixture()
def client():
...
def test_search_exception_returns_500(client):
    # raise_server_exceptions=False (set in fixture) lets the 500 surface as
    # an HTTP response rather than re-raising inside the test process.
